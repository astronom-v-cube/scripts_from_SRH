...
for fitsName in iFitses:
    ...
    mm = int(tHMS[1])
    ss = int(tHMS[2])
    time_0703_3100.append(hh*3600 + mm*60 + ss + int(tObs[1])*1e-6)
    fitsHandle.close()
...
flux_0703_3100 = []#iImages3100[:,305:345,410:435].mean(axis=(1,2))
Tbmax_0703_3100 = []
for i in range(40):
#    Tbmax3100.append(iImages3100[i,305:345,410:435].max())
    Tbmax_0703_3100.append(iImages3100[i,325,424].max())
    flux_0703_3100.append(2*iImages3100[i,305:345,410:435].sum()*scipy.constants.k/(scipy.constants.c/3.1e9)**2 * NP.deg2rad(4.911/3600)**2 / 1e-22)
