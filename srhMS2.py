    ...
    def initDataTable(self, srhFits, frequencyChannel, ewLcpPhaseCorrection, ewRcpPhaseCorrection, sLcpPhaseCorrection, sRcpPhaseCorrection, phaseCorrect = True, amplitudeCorrect = False):
        ...
        for scan in range(srhFits.dataLength):
            ...
            for vis in range(visibilityNumber):
                ...
                if phaseCorrect:
                    uv[0,0] *= NP.exp(1j*(srhFits.ewAntPhaLcp[j + 1] - srhFits.sAntPhaLcp[i + 1] + ewLcpPhaseCorrection[j] + sLcpPhaseCorrection[i]))
                    uv[0,1] *= NP.exp(1j*(srhFits.ewAntPhaRcp[j + 1] - srhFits.sAntPhaRcp[i + 1] + ewRcpPhaseCorrection[j] + sRcpPhaseCorrection[i]))
                if amplitudeCorrect:
                    uv[0,0] /= (srhFits.ewAntAmpLcp[j + 1] * srhFits.sAntAmpLcp[i + 1])
                    uv[0,1] /= (srhFits.ewAntAmpRcp[j + 1] * srhFits.sAntAmpRcp[i + 1])
