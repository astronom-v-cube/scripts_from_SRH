...
rcpArrSrc = phaseEdit.srhFits.ampRcp[:,:,:].copy()
lcpArrSrc = phaseEdit.srhFits.ampLcp[:,:,:].copy()
...
for ant in range(antennaNumber):
    for freq in range(freqNumber):
        ...
        try:
            ...
            if (psfCenter[ant,freq] > 5000 or psfCenter[ant,freq] < -5000):
...
psfOffstIndexes = []
for ii in range(207):
    val = deltaH/3600*psfCenter[ii,1]
    if (val < 0.98 and val > 0.84):
        psfOffstIndexes.append(ii)

psfOffsetAntennName = phaseEdit.srhFits.antennaNames[psfOffstIndexes]

PL.figure()
PL.title('SRH 20220315, offsets of the PSF Gaussian fit')
...
PL.plot(psfOffstIndexes,deltaH/3600*psfCenter[psfOffstIndexes,1],'+')
PL.legend()
PL.grid()
...
PL.figure()
PL.title('SRH 20220315, FWHM of the PSF Gaussian fit')
PL.gca().set_xticks(NP.arange(0,207,16) - 1)
...
PL.title('SRH 20220315, offsets and FWHM of the PSF Gaussian fit')
PL.gca().set_xticks(NP.arange(0,16,1))
PL.gca().set_xticklabels(xlabel)
PL.plot(deltaH/3600*psfCenter[32:,:].mean(axis=0),'-',label='offset')
PL.plot(deltaH/3600*psfWidth[32:,:].mean(axis=0),'-',label='FWHM')
PL.legend()
PL.grid()
PL.xlabel('GHz')
PL.ylabel('degree')
