...
fitNames = findFits('SRH36_temp_20210522_moon/','*.fit')  
fitNames.sort();
fitNames = fitNames[1:];

fdSrhImageName = open('/home/svlesovoi/SRH/srh_currentFileNameMoon.txt','w');
...
for ff in saveFreqs:
    sub.scatter(times[ff,0:ampLength],0.5*(ampFluxRcp[ff,:] + vNorm_amp[ff]*ampFluxLcp[ff,:]),color=c_list(ff), s=1.5, linewidths = 0,label='%d MHz'%(freqList[ff]*1e-3))
    sub.scatter(times[ff,0:ampLength],0.5*(ampFluxRcp[ff,:] - vNorm_amp[ff]*ampFluxLcp[ff,:]),color=c_list(ff), s=1.5, linewidths = 0)
...
hduList.writeto('srh_moon_' + dateName + '.fits',clobber=True);
hduList.close();
