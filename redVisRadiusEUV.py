...
#redVis = fits.open('srh_redVis_20200512.fits')
...
#f193 = fits.open('/home/svlesovoi/sunpy/data/aia20200512_000300_0193.fits')
...
#    ind = NP.where((theta > -30.5) * (theta < -26.5))
    ind = NP.where((theta > -32) * (theta < -26))
    ...
    leftHA.append(hA[ind[0][NP.argmin(fittedSubMean)]])
    srhTwoAntennaBeam.onCalc(leftHA[-1], RAO.declination, redFreq[0]*1e6, 1.8, 64, 66)
    beamSun = ndimage.zoom(srhTwoAntennaBeam.beamPattern[512-8:512+8,512-8:512+8],(64,64))
    leftBeam.append(beamSun)
    leftVisibility.append(NP.mean(beamSun*f193data))
    ...
    rightHA.append(hA[ind[0][NP.argmin(fittedSubMean)]])
    srhTwoAntennaBeam.onCalc(rightHA[-1], RAO.declination, redFreq[0]*1e6, 1.8, 64, 66)
    beamSun = ndimage.zoom(srhTwoAntennaBeam.beamPattern[512-8:512+8,512-8:512+8],(64,64))
    rightBeam.append(beamSun)
    rightVisibility.append(NP.mean(beamSun*f193data))
...
for f in range(len(minFreq)):
    xx =  x_size/2*NP.sin(leftGp[f] + pAngle*0)
    yy = -y_size/2*NP.cos(leftGp[f] + pAngle*0)
    PL.plot([-xx,xx],[-yy,yy],color='green',linewidth=0.3)
    xx =  x_size/2*NP.sin(rightGp[f] + pAngle*0)
    yy = -y_size/2*NP.cos(rightGp[f] + pAngle*0)
    PL.plot([-xx,xx],[-yy,yy],color='red',linewidth=0.3)

uvPerPixel = 1./(NP.deg2rad(x_size * 3600))
PL.imshow(f193data + beamSun*.0001,vmax=100,extent=[-x_size/2,x_size/2,y_size/2,-y_size/2])
