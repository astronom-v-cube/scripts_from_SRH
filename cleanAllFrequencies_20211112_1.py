...
msPath = '/home/sergeyvlesovoi/SRH_0612/20211112_1/'
for ff in range(16):
    print(ff)
...
with open(msPath + 'cleanAllFreqs.py', 'w') as f:
    f.write('freq0 = %d\n'%freq0)
    f.write('curDate = %s\n'%curDate)
    f.write('msPath = \'/home/sergeyvlesovoi/SRH_0612/20211112_1/srh%s_\'\n'%(curDate))
    f.write('imagePath = \'/home/sergeyvlesovoi/SRH_0612/20211112_1/images/cln_\'\n')
...
with open(msPath + 'saveFitses.py', 'w') as f:
    f.write('freq0 = %d\n'%freq0)
    f.write('imagePath = \'/home/sergeyvlesovoi/SRH_0612/20211112_1/images/cln_\'\n')
    f.write('clnFitsPath = \'/home/sergeyvlesovoi/SRH_0612/20211112_1/images/srh_%s_cln_\'\n'%(curDate))
    f.write('for curFreq in range(16):\n')
...
rcpImages = []
for ff in range(16):
    fitName = ('/home/sergeyvlesovoi/SRH_0612/20211112_1/images/srh_%s_cln_%d.fit'%(curDate,int(phaseEdit.srhFits.freqList[ff]*1e-3)))
    clnFits.append(fits.open(fitName))
    lcpImages.append(clnFits[-1][0].data[0,0])
    rcpImages.append(clnFits[-1][0].data[1,0])

lcpImages_20211112_1 = NP.array(lcpImages)
rcpImages_20211112_1 = NP.array(rcpImages)

Y0S = 286
X0S = 585
dXY = 128
srcLcpS = lcpImages_20211112_1[:,Y0S:Y0S+dXY,X0S:X0S+dXY]
srcRcpS = rcpImages_20211112_1[:,Y0S:Y0S+dXY,X0S:X0S+dXY]
Y0N = 688
X0N = 760
srcLcpN = lcpImages_20211112_1[:,Y0N:Y0N+dXY,X0N:X0N+dXY]
srcRcpN = rcpImages_20211112_1[:,Y0N:Y0N+dXY,X0N:X0N+dXY]
