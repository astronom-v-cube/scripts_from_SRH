...
fig.suptitle('SRH lm %s, %s, %s, %d MHz'%(date_obs, time_obs, 'I',fitsFile[1].data['frequency'][freq]/1e3))
#dW = 300
#X0 = 130
#Y0 = -320
dW = 300
...
VV = imagesR - imagesL
for scan in range(samplesNumber):
#    ims[scan//5,scan%5].imshow(NP.flip(images[scan,(uvSize - dW)//2 - X0:(uvSize + dW)//2 - X0,(uvSize - dW)//2 - Y0:(uvSize + dW)//2 - Y0], 1),cmap='gray')
    ims[scan//5,scan%5].imshow(NP.flip(II[scan,(uvSize - dW)//2 - X0:(uvSize + dW)//2 - X0,(uvSize - dW)//2 - Y0:(uvSize + dW)//2 - Y0], 1),cmap='gray')
    ims[scan//5,scan%5].contour(NP.flip(VV[scan,(uvSize - dW)//2 - X0:(uvSize + dW)//2 - X0,(uvSize - dW)//2 - Y0:(uvSize + dW)//2 - Y0], 1),cmap='gray_r',levels=[-0.4,0.4])
