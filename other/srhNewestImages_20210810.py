...
def prepareCleanImages(fileName, psfName, qSunTb):
    fd = fits.open(fileName)
    fd_psf = fits.open(psfName)
    contours = skimage.measure.find_contours(fd_psf[0].data[0,0], 0.5)
    sunEll = skimage.measure.EllipseModel()
    sunEll.estimate(contours[0])
    i_ex0, i_ey0, i_eA, i_eB, i_eT = sunEll.params
    gEll_patch.append(Ellipse((40,40), 2*i_eA, 2*i_eB, -i_eT/180*NP.pi, edgecolor='white', facecolor='none', linewidth=1.3))
...
for frequency in range(frequencyNumber):
    ...
    prepareCleanImages(saveName.split('.')[0] + '_clean_image.fit',saveName.split('.')[0] + '_clean_psf.fit',currentDiskTb)
...
for row in range(frequencyNumber):
    ...
    pl[row,0].add_patch(gEll_patch[row])
