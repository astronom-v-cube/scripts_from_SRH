...
parser = OptionParser()
parser.add_option("-d", "--date", dest="requestDate", default = '2021/06/25')
...
fig.subplots_adjust(left=0.1,right=0.9,top=0.95,bottom=0.01)
...
pl[1].set_xlim(0,600)
pl[1].set_ylim(0,367)
stereoData = stereoA['spectrum'].copy()
for f in range(86): stereoData[:,f] = stereoA['spectrum'][:,f] * 0.6
pl[1].imshow(stereoData.T,origin='lower',vmin=0, vmax=2., aspect=1/3)
pl[1].plot(srhTime[0]/60,(srhCorrI.mean(axis=0) - 0.002)*200000,color='white')
...
pl.plot(goesDatetime[smoothWindow:-smoothWindow],smooth(goesFile.variables[var_name[0]][:],smoothWindow)[smoothWindow:-smoothWindow],color='blue', label="{} {}".format(platform, var_name[0]))
pl.plot(goesDatetime[smoothWindow:-smoothWindow],smooth(goesFile.variables[var_name[1]][:],smoothWindow)[smoothWindow:-smoothWindow],color='red', label="{} {}".format(platform, var_name[1]))
pl.plot([srhDateTime[0],srhDateTime[-1]],[1e-7,1e-7],label = 'A class',color='gray')
pl.set_yscale("log")
pl.set_xlabel("Time [UT]")
pl.set_ylabel("X-Ray Flux [{}]".format(goesFile[var_name[0]].units))
pl.set_xlim(goesDatetime[14401 - 600],goesDatetime[14401 + 3600 - 600])
