...
#polarization signum
    ...
    if maxInds[0].shape[0] > 1:
        print('Sky level %f, Sun level %f'%(iImageHist[1][maxInds[0][0]],iImageHist[1][maxInds[0][1]]))
...
#------------------------------------------------------------------------------
parser = OptionParser()
parser.add_option("-f", "--file", dest="fitPath", default = '/home/maria/Work/SRH imaging/3-6/fits/20210430')
...
for fileName in fitNames:
    ...
    for frequency in range(frequencyNumber):
        ...
        im[0][0] = model * file.diskLevelLcp * 1.2e-3
        im[0][1] = model * file.diskLevelRcp * 1.2e-3
        
        casa_im.putdata(im)
        casa_im.saveas('images/disk2.model', overwrite=True)
        casa_im.unlock()
