...
fitNames = findFits('/home/svlesovoi/SRH_DATA/SRH36/20210319','*.fit')
fitNames.sort()
...
images = NP.zeros((freqListLength,uvSize,uvSize))
uves = NP.zeros((freqListLength,130,130))

for freq in range(freqListLength):
#    for scan in range(visLcp.shape[1]):
    for scan in range(1):
...
#        ewAntPha[flagsEW] = 0
        ...
        images[freq,:,:] = image.real
        showUV = NP.abs(uv[uvSize//2-65:uvSize//2+65,uvSize//2-65:uvSize//2+65])
        showUV = NP.clip(showUV,0.0,0.1*NP.max(showUV))
        showUV = NP.roll(showUV,-1,axis=1)
        showUV = NP.roll(showUV,-1,axis=0)
        uves[freq,:,:] += showUV    

fig = PL.figure(figsize=(16,4))
fig.suptitle('SRH lm %s, %s, %s'%(date_obs, time_obs, 'RCP' if polarization  else 'LCP'))
fig.subplots_adjust(left=0.01,right=.99,wspace=0.01, hspace=0.01, bottom=0.01)
fig.tight_layout()
ims = fig.subplots(1,freqListLength)
for freq in range(freqListLength):
    ims[freq].imshow(NP.flip(images[freq,:,:], 1),cmap='hot')
    ims[freq].set_title('%d MHz'%(fitsFile[1].data['frequency'][freq]/1e3))
    ims[freq].axis('off')

fig = PL.figure(figsize=(16,4))
fig.suptitle('SRH uv %s, %s, %s'%(date_obs, time_obs, 'RCP' if polarization  else 'LCP'))
fig.subplots_adjust(left=0.01,right=.99,wspace=0.01, hspace=0., bottom=0.01)
fig.tight_layout()
ims = fig.subplots(1,freqListLength)
for freq in range(freqListLength):
    ims[freq].set_title('%d MHz'%(fitsFile[1].data['frequency'][freq]/1e3))
    ims[freq].imshow(uves[freq,:,:],cmap='hot')
    ims[freq].axis('off')
