...
for fName in fitName:
  ...
  if i == 0:
    ...
    for vis in range(0,55):
      sub_vis_rcp[vis,:,:] = hdu[1].data.field(2*vis + 20);
      sub_vis_lcp[vis,:,:] = hdu[1].data.field(2*vis + 21);
    vis_rcp = sub_vis_rcp;
    vis_lcp = sub_vis_lcp;
  else:
    if i == len(fitName) - 1:
...
sky_lcp = zeros((10,F),'float');
sky_rcp = zeros((10,F),'float');
sun_lcp = zeros((10,F),'float');
sun_rcp = zeros((10,F),'float');

for i in range(0,F):
  for j in range(0,10):
	sky_lcp[j,i] = abs(vis_lcp[j,i,sky_t0:sky_t1]).mean();
	sky_rcp[j,i] = abs(vis_rcp[j,i,sky_t0:sky_t1]).mean();
	sun_lcp[j,i] = (abs(vis_lcp[j,i,sun_t0:sun_t1]) - sky_lcp[j,i]).mean();
	sun_rcp[j,i] = (abs(vis_rcp[j,i,sun_t0:sun_t1]) - sky_rcp[j,i]).mean();

fdClbSkyLcp = open('clb_sky_lcp.txt','wb');
fdClbSunLcp = open('clb_sun_lcp.txt','wb');
fdClbSkyRcp = open('clb_sky_rcp.txt','wb');
fdClbSunRcp = open('clb_sun_rcp.txt','wb');
for i in range(0,F):
	fdClbSkyLcp.write("%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n" % (sky_lcp[0,i],sky_lcp[1,i],sky_lcp[2,i],sky_lcp[3,i],sky_lcp[4,i],sky_lcp[5,i],sky_lcp[6,i],sky_lcp[7,i],sky_lcp[8,i],sky_lcp[9,i]));
	fdClbSunLcp.write("%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n" % (sun_lcp[0,i],sun_lcp[1,i],sun_lcp[2,i],sun_lcp[3,i],sun_lcp[4,i],sun_lcp[5,i],sun_lcp[6,i],sun_lcp[7,i],sun_lcp[8,i],sun_lcp[9,i]));
	fdClbSkyRcp.write("%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n" % (sky_rcp[0,i],sky_rcp[1,i],sky_rcp[2,i],sky_rcp[3,i],sky_rcp[4,i],sky_rcp[5,i],sky_rcp[6,i],sky_rcp[7,i],sky_rcp[8,i],sky_rcp[9,i]));
	fdClbSunRcp.write("%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f\n" % (sun_rcp[0,i],sun_rcp[1,i],sun_rcp[2,i],sun_rcp[3,i],sun_rcp[4,i],sun_rcp[5,i],sun_rcp[6,i],sun_rcp[7,i],sun_rcp[8,i],sun_rcp[9,i]));
fdClbSkyLcp.close();
fdClbSunLcp.close();
fdClbSkyRcp.close();
fdClbSunRcp.close();
...
dt = tStopGlobal - tStartGlobal;
dt = (dt.seconds + dt.microseconds*1e-6)/(1.*L);
dt_major = 3600.;
dt_minor = 900.;
t0_axis_hours = 0.;
t1_axis_hours = 10.;
...
commonTitle = 'Siberian radioheliograph prototype, ' + tStartGlobal.strftime('%Y %B %d');
...
ant_min = min(ant_i[0,ant_freq,T0:]);
ant_max = max(ant_i[0,ant_freq,T0:]);
norm_ant_i0 = (ant_i[0,ant_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(ant_i[1,ant_freq,T0:]);
ant_max = max(ant_i[1,ant_freq,T0:]);
norm_ant_i1 = (ant_i[1,ant_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(ant_i[4,ant_freq,T0:]);
ant_max = max(ant_i[4,ant_freq,T0:]);
norm_ant_i4 = (ant_i[4,ant_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(ant_i[5,ant_freq,T0:]);
ant_max = max(ant_i[5,ant_freq,T0:]);
norm_ant_i5 = (ant_i[5,ant_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(vis_flux_lcp[0,vis_freq,T0:] + vis_flux_rcp[0,vis_freq,T0:]);
ant_max = max(vis_flux_lcp[0,vis_freq,T0:] + vis_flux_rcp[0,vis_freq,T0:]);
norm_vis_i0 = (vis_flux_lcp[0,vis_freq,T0:] + vis_flux_rcp[0,vis_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(vis_flux_lcp[1,vis_freq,T0:] + vis_flux_rcp[1,vis_freq,T0:]);
ant_max = max(vis_flux_lcp[1,vis_freq,T0:] + vis_flux_rcp[1,vis_freq,T0:]);
norm_vis_i1 = (vis_flux_lcp[1,vis_freq,T0:] + vis_flux_rcp[1,vis_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(vis_flux_lcp[4,vis_freq,T0:] + vis_flux_rcp[4,vis_freq,T0:]);
ant_max = max(vis_flux_lcp[4,vis_freq,T0:] + vis_flux_rcp[4,vis_freq,T0:]);
norm_vis_i4 = (vis_flux_lcp[4,vis_freq,T0:] + vis_flux_rcp[4,vis_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_min = min(vis_flux_lcp[5,vis_freq,T0:] + vis_flux_rcp[5,vis_freq,T0:]);
ant_max = max(vis_flux_lcp[5,vis_freq,T0:] + vis_flux_rcp[5,vis_freq,T0:]);
norm_vis_i5 = (vis_flux_lcp[5,vis_freq,T0:] + vis_flux_rcp[5,vis_freq,T0:] - ant_min)/(ant_max - ant_min);

ant_avg0 = (norm_ant_i0 + norm_ant_i1 + norm_ant_i4 + norm_ant_i5)/4.;
vis_avg0 = (norm_vis_i0 + norm_vis_i1 + norm_vis_i4 + norm_vis_i5)/4.;
...
ant_avg1 = (norm_ant_i0 + norm_ant_i1 + norm_ant_i4 + norm_ant_i5)/4.;
vis_avg1 = (norm_vis_i0 + norm_vis_i1 + norm_vis_i4 + norm_vis_i5)/4.;
