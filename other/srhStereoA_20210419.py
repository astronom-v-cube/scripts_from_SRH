...
parser = OptionParser();
parser.add_option("-d", "--date", dest="inDate",   default="20210419");
...
goesFits = fits.open('go1320170422.fits')
...
PL.figure()
PL.title('20170422, 04:08 UT')
PL.ylabel('flux s.f.u.')
PL.xlabel('frequency MHz')
PL.grid()
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-0] - cpI[:,t1-5])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-1] - cpI[:,t1-5])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-2] - cpI[:,t1-5])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-2] - cpI[:,t1-5])*NP.linspace(5,10,5)*100,label='I')
PL.plot(cpFits[1].data['frequencies'],(cpV[:,t1-2]/(cpI[:,t1-2] - cpI[:,t1-5]))*10,label='V/I*10')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-3] - cpI[:,t1-5])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-4] - cpI[:,t1-5])*NP.linspace(5,10,5)*100,'o')
PL.legend()
