...
parser = OptionParser()
parser.add_option("-f", "--file", dest="fitPath", default = 'SRH36_temp_20210815_2/')
parser.add_option("-t", "--treshold", dest="cleanTresh", default = '0.2mJy')
parser.add_option("-s", "--scan", dest="scan", default = '0~3')
...
#        pass
    ...
    saveName = 'MS30/' + fileName.split('/')[-1].split('.')[0] + '.ms'
    ...
    prepareCleanImages(saveName.split('.')[0] + '_clean_image.fit',saveName.split('.')[0] + '_clean_psf.fit', currentDiskTb, 'cleanMaps/' + currentDate + '/',file, frequency)
