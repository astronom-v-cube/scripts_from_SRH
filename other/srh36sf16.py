...
fitsNames = findFits('/home/svlesovoi/SRH_0306/' + dateName ,'*.fit')
fitsNames.sort()
...
srhFits.calculatePhaseCalibration(baselinesNumber = srhFits.baselines, rcp = False)
...
#    srhFits.baselines = baseLines[frequencyChannel]
    srhFits.vis2uv(scan, amplitudeCorrect = True, average = 20);
    srhFits.uv2lmImage()
...
PL.yticks([])
dateName = DT.datetime.now().strftime("%Y%m%d")
#pngName = 'srh_'+srhFits.dateObs+'_'+str(scan)+'_nonlin3_iter150.png'
