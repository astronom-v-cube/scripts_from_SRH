...
parser = OptionParser();
parser.add_option("-d", "--date", dest="inDate",   default="20170402");
...
goesFits = fits.open('go1320170402.fits')
...
#    pl0.plot(cpTime[i][t0:t1], cpI[i][t0:t1],'.', color='yellow')
    pl0.plot(cpTime[i], cpV[i], color=PL.cm.gray(int(i/freqs*200)))
pl0.plot(goesTime[0,:],goesFlux[0,:,0]*1e3,color='red')
pl0.plot(goesTime[0,:],goesFlux[0,:,1]*1e3,color='blue')
pl0.legend()
...
pl1.plot(goesTime[0,:]/60,goesFlux[0,:,0]*5e6,color='white')
pl1.plot(goesTime[0,:]/60,goesFlux[0,:,1]*5e6,color='white')
goesFits.close()

PL.figure()
PL.title('20170402, 07:54 UT')
PL.ylabel('flux s.f.u.')
PL.xlabel('frequency MHz')
PL.grid()
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-0] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-3] - cpI[:,t0])*NP.linspace(5,10,5)*100)
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-1] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-2] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-3] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t1-4] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t0+1] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t0+2] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t0+3] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
PL.plot(cpFits[1].data['frequencies'],(cpI[:,t0+4] - cpI[:,t0])*NP.linspace(5,10,5)*100,'o')
