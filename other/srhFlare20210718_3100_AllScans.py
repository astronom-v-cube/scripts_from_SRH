...
for fitsName in iFitses:
    ...
    mm = int(tHMS[1])
    ss = int(tHMS[2])
    time_0718_3100.append(hh*3600 + mm*60 + ss + int(tObs[1])*1e-6)
    fitsHandle.close()
...
fig.suptitle('SRH 20210703, 07:16-07:17 UT, 3.1 GHz')
for row in range(nRows):
    for col in range(nCols):
        pl[row,col].axis('off')
        pl[row,col].imshow(iImages3100[row*nCols + col,y0:y1,x0:x1],origin='lower',cmap='hot',vmin=0,vmax=1e7)

flux_0718_3100 = []
Tbmax_0718_3100 = []
for i in range(frames):
    Tbmax_0718_3100.append(iImages3100[i,374,313])
    flux_0718_3100.append(2*iImages3100[i,341:402,290:340].sum()*scipy.constants.k/(scipy.constants.c/3.4e9)**2 * NP.deg2rad(4.911/3600)**2 / 1e-22)
