...
#    ell_patch = Ellipse((i_ey0,i_ex0), 2*i_eA, 2*i_eB, -i_eT/180*NP.pi, edgecolor='green', facecolor='none', linewidth=1.3)
    ...
    global gSaveVname
    gSaveVname = 'srh_V_%s.png'%fd[0].header['DATE-OBS'].split('.')[0] 
    fig.savefig(gSaveVname)
...
#    pl.add_patch(ell_patch)
    ...
    global gSaveIname
    gSaveIname = 'srh_I_%s_%s.png'%(fd[0].header['DATE-OBS'].split('.')[0],cleanTresh)
    fig.savefig(gSaveIname)
