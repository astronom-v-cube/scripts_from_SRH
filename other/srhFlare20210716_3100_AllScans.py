...
iFitses = srh36Utils.findFits('cleanMaps/20210703/snapshot_0716/%d'%frequency,'*I*.fit')
vFitses = srh36Utils.findFits('cleanMaps/20210703/snapshot_0716/%d'%frequency,'*V*.fit')
...
for fitsName in iFitses:
    ...
    mm = int(tHMS[1])
    ss = int(tHMS[2])
    time_0716_3100.append(hh*3600 + mm*60 + ss + int(tObs[1])*1e-6)
    fitsHandle.close()
...
for row in range(nRows):
    for col in range(nCols):
        pl[row,col].axis('off')
        pl[row,col].imshow(iImages3100[row*nCols + col],origin='lower',cmap='hot',vmin=0,vmax=5e5)

flux_0716_3100 = []
Tbmax_0716_3100 = []
for i in range(frames):
    Tbmax_0716_3100.append(iImages3100[i,332,424])
    flux_0716_3100.append(2*iImages3100[i,309:353,407:442].sum()*scipy.constants.k/(scipy.constants.c/3.4e9)**2 * NP.deg2rad(4.911/3600)**2 / 1e-22)
...
pl.set_title('SRH 20210703 07:16 UT 3.1 GHz I image')
pl.set_xlabel('arcmin')
pl.set_ylabel('arcmin')
pl.grid(linestyle='--')
pl.imshow(iImages.mean(axis=0),origin='lower',cmap='hot',vmin=10000, vmax=1e6)
pl.contour(iImages.mean(axis=0),origin='lower',levels=[1.5e6,2.5e6,3.5e6,6.5e6,7.5e6,10.5e6])
