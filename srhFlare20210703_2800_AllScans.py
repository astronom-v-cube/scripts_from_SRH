...
for fitsName in iFitses:
    ...
    mm = int(tHMS[1])
    ss = int(tHMS[2])
    time_0703_2800.append(hh*3600 + mm*60 + ss + int(tObs[1])*1e-6)
    fitsHandle.close()
...
iHist = NP.histogram(iImages.mean(axis=0),10000)
PL.figure()
PL.xlim(-1e5,1e5)
...
flux_0703_2800 = []#iImages2800[:,305:345,410:435].mean(axis=(1,2))
Tbmax_0703_2800 = []
for i in range(40):
#    Tbmax2800.append(iImages2800[i,305:345,410:435].max())
    Tbmax_0703_2800.append(iImages2800[i,325,424].max())
    flux_0703_2800.append(2*iImages2800[i,305:345,410:435].sum()*scipy.constants.k/(scipy.constants.c/2.8e9)**2 * NP.deg2rad(4.911/3600)**2 / 1e-22)
