...
for fitsName in iFitses:
    ...
    mm = int(tHMS[1])
    ss = int(tHMS[2])
    time_0718_3400.append(hh*3600 + mm*60 + ss + int(tObs[1])*1e-6)
    fitsHandle.close()
...
for i in range(len(iFitses)):
    iI = iImages[i].copy()
    iHist.append(NP.histogram(iImages[i],10000))
...
for i in range(len(iFitses)): 
    ...
    peaks, _ = scipy.signal.find_peaks(iHist[i][0],prominence = 150)
    ind1 = peaks[0]
...
fig.subplots_adjust(left=0.01,right=0.99,top=0.9,bottom=0.01,hspace=0.01,wspace=0.01)
fig.suptitle('SRH 20210703, 07:16-07:17 UT, 3.4 GHz')
for row in range(nRows):
    for col in range(nCols):
        pl[row,col].axis('off')
        pl[row,col].imshow(iImages3400[row*nCols + col,y0:y1,x0:x1],origin='lower',cmap='hot',vmin=0,vmax=1e7)

flux_0718_3400 = []
Tbmax_0718_3400 = []
for i in range(frames):
    Tbmax_0718_3400.append(iImages3400[i,358,324])
    flux_0718_3400.append(2*iImages3400[i,336:372,310:345].sum()*scipy.constants.k/(scipy.constants.c/3.4e9)**2 * NP.deg2rad(4.911/3600)**2 / 1e-22)
