...
for fitsName in iFitses:
    ...
    mm = int(tHMS[1])
    ss = int(tHMS[2])
    time_0716_3400.append(hh*3600 + mm*60 + ss + int(tObs[1])*1e-6)
    fitsHandle.close()
...
for row in range(nRows):
    for col in range(nCols):
        pl[row,col].axis('off')
        pl[row,col].imshow(iImages3400[row*nCols + col],origin='lower',cmap='hot',vmin=0,vmax=5e5)

flux_0716_3400 = []
Tbmax_0716_3400 = []
for i in range(frames):
    Tbmax_0716_3400.append(iImages3400[i,358,324])
    flux_0716_3400.append(2*iImages3400[i,336:372,310:345].sum()*scipy.constants.k/(scipy.constants.c/3.4e9)**2 * NP.deg2rad(4.911/3600)**2 / 1e-22)
