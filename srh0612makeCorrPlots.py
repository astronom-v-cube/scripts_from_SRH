...
try:
    previousCpFits = fits.open('srh_0612_cp_' + dateName + '.fits')
...
currentRawFitses = srh0612Utils.findFits('SRH0612/' + dateName ,'*.fit')  
...
try:
    previousCpZerosFits = fits.open('srh_0612_cp_zeros.fits')
...
try:
    previousCpFluxNormFits = fits.open('srh_0612_cp_fluxNorm.fits')
...
sub.set_xlim((2.*3600.,8.*3600.))
sub.set_ylim((-0.01,0.09))
for ff in saveFreqs:
...
hduList.writeto('srh_0612_cp_' + dateName + '.fits',clobber=True);
hduList.close();
...
#hduList.writeto('srh_0612_cp_zeros_' + dateName + '.fits',clobber=True);
#hduList.close();
...
#pHdu = fits.PrimaryHDU(header=pHeader);
#hduList = fits.HDUList([pHdu, fTableHdu, dTableHdu]);
#hduList.writeto('srh_0612_cp_fluxNorm_' + dateName + '.fits',clobber=True);
#hduList.close();
...
fi = open('srh_0612_cp_' + dateName + '.fits','rb');
fd.storbinary('STOR /Public/sergey/corrPlots/' + 'srh_0612_cp_' + dateName + '.fits',fi);
fi.close();
